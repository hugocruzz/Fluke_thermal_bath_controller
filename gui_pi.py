...
class MainWindow:
    def __init__(self, root):
        ...
        # Adjust window size based on platform
        self.is_raspberry_pi = 'arm' in platform.machine().lower() and os.path.exists('/sys/firmware/devicetree/base/model')
        
        if self.is_raspberry_pi:
            # Smaller default size and font adjustments for Pi
            self.root.geometry("800x480")  # Common resolution for Pi touchscreens
            default_font = tk.font.nametofont("TkDefaultFont")
            default_font.configure(size=9)  # Smaller font for Pi
        ...
        self.config_dir = os.path.join(script_dir, "configs")
    ...
    def _create_ui(self):
        ...
        experiment_name_entry = ttk.Entry(top_frame, textvariable=self.experiment_name_var, width=25 if self.is_raspberry_pi else 30)
        experiment_name_entry.pack(side=tk.LEFT, padx=(0, 10))
        
        # Control buttons frame
        control_buttons = ttk.Frame(top_frame)
        control_buttons.pack(side=tk.LEFT)
        
        # Create buttons in grid for better layout on small screens
        self.start_button = ttk.Button(control_buttons, text="Start", command=self.start_experiment)
        self.start_button.grid(row=0, column=0, padx=3, pady=2)
        
        self.pause_resume_button = ttk.Button(control_buttons, text="Pause", command=self.toggle_pause_resume, state=tk.DISABLED)
        self.pause_resume_button.grid(row=0, column=1, padx=3, pady=2)
        
        self.stop_button = ttk.Button(control_buttons, text="Stop", command=self.stop_experiment, state=tk.DISABLED)
        self.stop_button.grid(row=0, column=2, padx=3, pady=2)
        
        self.reset_button = ttk.Button(control_buttons, text="Reset", command=self.reset_experiment)
        self.reset_button.grid(row=1, column=0, padx=3, pady=2)
        
        self.load_config_button = ttk.Button(control_buttons, text="Load Config", command=self.load_config_file)
        self.load_config_button.grid(row=1, column=1, padx=3, pady=2)
        ...
        left_frame = ttk.LabelFrame(middle_frame, text="Settings", padding="5")
        ...
        default_port = "COM10" if platform.system() == "Windows" else "/dev/ttyUSB0"
        
        ttk.Label(serial_frame, text="Port:").grid(row=0, column=0, sticky=tk.W, padx=5)
        ...
        ttk.Entry(temp_frame, textvariable=self.setpoint_var, width=8).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Entry(stability_frame, textvariable=self.stability_window_var, width=entry_width).grid(row=0, column=3, sticky=tk.W, padx=5)
        
        ttk.Label(stability_frame, text="Read Interval (s):").grid(row=1, column=0, sticky=tk.W, padx=5)
        self.reading_interval_var = tk.DoubleVar(value=5.0)
        ttk.Entry(stability_frame, textvariable=self.reading_interval_var, width=entry_width).grid(row=1, column=1, sticky=tk.W, padx=5)
        ...
        ttk.Entry(stability_frame, textvariable=self.timeout_duration_var, width=entry_width).grid(row=1, column=3, sticky=tk.W, padx=5)
        ...
        ttk.Entry(stability_frame, textvariable=self.min_readings_var, width=entry_width).grid(row=2, column=1, sticky=tk.W, padx=5)
        ...
        setpoints_frame = ttk.LabelFrame(right_frame, text="Temperature Steps", padding="5")
        ...
        setpoint_buttons_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=5)
        ...
        self.status_text = scrolledtext.ScrolledText(bottom_frame, wrap=tk.WORD, height=8 if self.is_raspberry_pi else 10)
        ...
        ttk.Label(status_bar, textvariable=self.current_temp_var, font=("Arial", 9 if self.is_raspberry_pi else 10, "bold")).pack(side=tk.LEFT)
        
        # Current step display
        ttk.Label(status_bar, text="   Current Step:").pack(side=tk.LEFT, padx=(10, 5))
        self.current_step_var = tk.StringVar(value="--")
        ttk.Label(status_bar, textvariable=self.current_step_var, font=("Arial", 9 if self.is_raspberry_pi else 10, "bold")).pack(side=tk.LEFT)
    ...
    def scan_com_ports(self):
        ...
        try:
            ...
            ttk.Label(port_window, text="Available Ports:", 
                     font=("Arial", 10 if self.is_raspberry_pi else 12, "bold")).pack(pady=5)
            ...
            port_listbox = tk.Listbox(list_frame, font=("Courier New", 8 if self.is_raspberry_pi else 10), 
            ...
            for i, port in enumerate(ports):
                ...
                if platform.system() == "Windows" and "COM" in port_name:
                    port_listbox.selection_set(i)
                elif platform.system() != "Windows" and ("USB" in port.description or "Serial" in port.description):
                    port_listbox.selection_set(i)
            ...
            # Log the scan
            self.log_message(f"Found {len(ports)} ports")
...
# Launcher script that works on both Windows and Raspberry Pi
def main():
    root = tk.Tk()
    app = MainWindow(root)
    
    # Add platform detection for fullscreen mode on Raspberry Pi touchscreens
    if 'arm' in platform.machine().lower() and os.path.exists('/sys/firmware/devicetree/base/model'):
        try:
            # Check if it's a touchscreen setup
            with open('/sys/firmware/devicetree/base/model', 'r') as f:
                model = f.read()
                if "Raspberry Pi" in model:
                    # Optional: Uncomment to run in fullscreen on Pi touchscreens
